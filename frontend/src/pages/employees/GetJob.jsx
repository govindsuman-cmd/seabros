  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        // Assuming the API returns total count and pages info
      ...
      {jobs.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {jobs.map((job) => (
            <div
              key={job._id}
              className="p-6 border border-blue-100 rounded-xl bg-white shadow-sm hover:shadow-md transition-all duration-300 hover:border-blue-300 cursor-pointer"
